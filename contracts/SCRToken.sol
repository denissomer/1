...
contract SCRToken is ERC20, SafeMath, Ownable {
   ...
   // metadata
    string  public constant name = "EWM SCR Token";
    ...
    address public EWMPlatform;


    function setHost(address _EWMPlatform) onlyOwner {
       EWMPlatform = _EWMPlatform;
    }
    ...
    function mintToken(address who, uint256 value) internal {
        require((msg.sender==crowdSale) || (msg.sender == EWMPlatform));
        require(who != 0x0);
    ...
    function burnToken(address who, uint256 value) internal{
        require(msg.sender == EWMPlatform);
        require (who != 0x0);
